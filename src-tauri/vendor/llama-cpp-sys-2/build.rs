...
fn main() {
    ...
    // Temporary mitigation: clear C/C++ compile-related env vars that
    // may be incorrectly injected (e.g. CXXFLAGS=/utf-8) which breaks
    // compiler invocations on macOS in some environments.
    std::env::remove_var("CXXFLAGS");
    std::env::remove_var("CFLAGS");
    std::env::remove_var("CXX");
    std::env::remove_var("CC");
    println!("cargo:warning=build.rs: cleared CXXFLAGS/CFLAGS/CXX/CC env vars (temporary patch)");
