...
impl InferenceCache {
    ...
    /// 获取缓存结果
    pub fn get(&self, prompt: &str, params: &str) -> Option<String> {
    ...
    /// 设置缓存
    pub fn set(&self, prompt: &str, params: &str, result: &str) {
        let key = self.generate_key(prompt, params);
        ...
        eprintln!("[缓存] 存储: key={}, size={}", key, result.len());
    ...
    pub fn stats(&self) -> CacheStats {
        ...
        let mut hits_by_entry: Vec<u32> = cache.values().map(|e| e.hit_count).collect();

        CacheStats {
            entries: cache.len(),
            total_hits,
            max_size: self.max_size,
            avg_hits: if cache.is_empty() { 0.0 } else {
...
mod tests {
    ...
    fn test_cache_basic() {
        ...
        // 测试存储和获取
        cache.set("test prompt", "temp:0.7", "test result");
    ...
    fn test_cache_eviction() {
        ...
        cache.set("prompt1", "temp:0.7", "result1");
        cache.set("prompt2", "temp:0.7", "result2");
        cache.set("prompt3", "temp:0.7", "result3"); // 应该清除 prompt1

        assert_eq!(cache.get("prompt1", "temp:0.7"), None);
        assert_eq!(cache.get("prompt2", "temp:0.7"), Some("result2".to_string()));
        assert_eq!(cache.get("prompt3", "temp:0.7"), Some("result3".to_string()));
    }
    ...
    fn test_truncate_messages() {
        ...
        let messages = vec![
            Message { role: "user".to_string(), content: "A".repeat(100) },
            Message { role: "assistant".to_string(), content: "B".repeat(100) },
            Message { role: "user".to_string(), content: "C".repeat(100) },
        ];
